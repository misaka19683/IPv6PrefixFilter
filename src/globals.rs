...
//pub static INTERFACE_LIST: Lazy<Mutex<Vec<NetworkInterface>>> = Lazy::new(|| Mutex::new(Vec::new()));
...
// INTERFACE_NAME 方法
/// 设置全局接口名称。
pub fn set_interface_name(interface_name: String) {
    ...
    let mut contain = INTERFACE_NAME.lock().unwrap();
    *contain = Some(interface);
}
